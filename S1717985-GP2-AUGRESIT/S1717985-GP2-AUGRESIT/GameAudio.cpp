...
int GameAudio::convertToInt(char* buffer, int length)
{
	int a = 0;
	if (!isBigEndian())
...
char* GameAudio::loadWAV(const char* fn, int& chan, int& samplerate, int& bps, int& size)
{
	char buffer[4];
...
unsigned int GameAudio::loadSound(const char* filename)
{
	bool found = false;
...
void GameAudio::deleteSound(unsigned int id)
{}

void GameAudio::playSound(unsigned int id)
{
	alSourcePlay(id);
}
void GameAudio::playSound(unsigned int id, glm::vec3& pos)
{
	alSource3f(id, AL_POSITION, pos.x, pos.y, pos.z);
	alSourcePlay(id);
}
void GameAudio::stopSound(unsigned int id)
{
	alSourceStop(id);
}
void GameAudio::setlistener(glm::vec3& pos, glm::vec3& camLookAt)
{
